...
mod tests {
    ...
    #[tokio::test]
    async fn dropped_ping_reply_is_not_success() {
        let (sender, mut receiver) = channel(1);
        let (reply_sender, reply_receiver) = unbounded_channel();
        drop(reply_sender);
        let handle = Handle::<TestHandler> {
            sender,
            receiver: reply_receiver,
            join: russh_util::runtime::spawn(async { Ok(()) }),
            channel_buffer_size: 1,
        };
        let worker = tokio::spawn(async move {
            match receiver.recv().await {
                Some(Msg::Ping { reply_channel }) => drop(reply_channel),
                _ => panic!("expected ping request"),
            }
        });
        assert!(matches!(handle.send_ping().await, Err(Error::SendError)));
        worker.await.expect("ping worker");
    }
