...
impl WindowsWindowInner {
    pub(crate) fn revoke_drag_drop(&self) {
        if self.drag_drop_registered.get() {
            let result = unsafe { RevokeDragDrop(self.hwnd) };
            if result.is_ok() {
...
fn register_drag_drop(window: &Rc<WindowsWindowInner>) -> Result<()> {
    ...
    window.drag_drop_registered.set(true);
    Ok(())
}
